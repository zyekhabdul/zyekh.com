...
# Font Resolver
def get_font(size=20, is_mono=False, is_bold=False):
    mono_fonts = [
...
def generate_social_card(article_data, output_path: Path):
    ...
    # Outer Card Container (Margin 40px)
    margin = 40
    card_rect = [margin, margin, width - margin, height - margin]
    draw.rectangle(card_rect, fill=card_color, outline=border_color, width=2)

    # Fonts
    font_tag = get_font(size=18, is_mono=True, is_bold=True)
    font_title = get_font(size=34, is_mono=False, is_bold=True)
    font_body = get_font(size=20, is_mono=False, is_bold=False)
    font_code = get_font(size=18, is_mono=True, is_bold=False)
    font_meta = get_font(size=16, is_mono=True, is_bold=False)

    tags = article_data.get('tags', ['SECURITY'])
    cat_tag = f"[ {tags[0].upper()} HARDENING ]" if tags else "[ TECHNICAL SPECIFICATION ]"
    
    # 1. Top Category Tag
    draw.text((margin + 35, margin + 35), cat_tag, fill=text_muted, font=font_tag)

    # 2. Main Title (Wrapped max 2 lines)
    title_lines = wrap_text(article_data['title'], font_title, width - (margin * 2 + 70), draw)
    title_lines = title_lines[:2]
    
    curr_y = margin + 70
    for line in title_lines:
        draw.text((margin + 35, curr_y), line, fill=text_main, font=font_title)
        curr_y += 44
    ...
    curr_y += 15
    inner_w = width - (margin * 2 + 70)

    if standard == "A":
        # Standar A: Kernel Config Matrix
        box_h = 160
        box_rect = [margin + 35, curr_y, margin + 35 + inner_w, curr_y + box_h]
        draw.rectangle(box_rect, fill=box_bg, outline=border_color, width=1)
        ...
        for cl in code_lines:
            ...
            draw.text((margin + 50, cy), cl, fill=c_color, font=font_code)
            cy += 32

        curr_y += box_h + 20
        draw.text((margin + 35, curr_y), "Impact: Zero-Trust Process Sandboxing & Syscall Isolation at Kernel Level", fill=text_muted, font=font_body)
        ...
        for fl in flow_lines:
            draw.text((margin + 50, cy), fl, fill=text_main, font=font_code)
            cy += 32

        curr_y += box_h + 20
        draw.text((margin + 35, curr_y), "Performance: Line-Rate Packet Filtering with Zero CPU Stack Overhead", fill=text_muted, font=font_body)
        ...
        for tl in tbl_lines:
            ...
            draw.text((margin + 50, cy), tl, fill=c_color, font=font_code)
            cy += 32

        curr_y += box_h + 20
        draw.text((margin + 35, curr_y), "Key Innovation: Virtual Memory Paging Eliminates Memory Fragmentation", fill=text_muted, font=font_body)
        ...
        for ul in util_lines:
            draw.text((margin + 50, cy), ul, fill=text_main, font=font_code)
            cy += 32

        curr_y += box_h + 20
        draw.text((margin + 35, curr_y), "Transformation Matrix: Local Cryptographic & Octal Permission Evaluation", fill=text_muted, font=font_body)

    # Footer Specification Line (No Brand Logo / No Ad Signal)
    draw.text((margin + 35, height - margin - 35), "Ref: High-Density Technical Reference Specification", fill=text_muted, font=font_meta)

    img.save(output_path, "PNG")
    print(f"[ SUCCESS ] Social Share Card Generated ({standard}): {output_path}")
    return output_path
...
def main():
    parser = argparse.ArgumentParser(description="Zyekh.com Category Social Share Card Generator")
    ...
    if args.slug:
        ...
        print(f"[ BATCH ] Generating social cards for {len(articles)} articles...")
        for a in articles:
