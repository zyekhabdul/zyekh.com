...
- **Multi-Persona Syndication & Dynamic Canvas Budgeting (Protocol 51)**: `generate_social_cards.py` dynamically computes available canvas height to scale box padding and line heights, and `syndicate.py` provides multi-persona copy angles (`authority`, `curation`, `quick_tip`, `auto`) and jittered delay (1.5s-4.5s) to avoid remote rate limiting.
- **Pre-Proposal Filesystem Discovery & Anti-Redundancy Standard (Protocol 52)**: Before proposing ANY new script, workflow, tool, or CI/CD task, the AI Agent MUST explicitly survey target directories (`.github/workflows/`, `scripts/`, `tools/`, `blueprints/`) via `list_dir` or `grep_search` to verify if the feature already exists under the same or similar filename. Proposing duplicate or phantom work without checking existing code is strictly forbidden.
