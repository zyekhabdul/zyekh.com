  ...
  function initCmdPalette() {
    ...
    dialog.innerHTML = `
      <div class="cmd-container">
        <input type="text" id="cmdInput" class="cmd-input" placeholder="Search 53+ tools, 35+ articles, AI chat (Ctrl+K)..." autocomplete="off" role="combobox" aria-autocomplete="list" aria-expanded="true" aria-controls="cmdResults" aria-label="Search tools and articles">
    ...
    function renderList(items, sectionTitle) {
      ...
      let html = '';
      if (sectionTitle) {
        html += `<div class="cmd-section-title">${sectionTitle}</div>`;
      }
      html += items.map(item => `
        <a href="${item.url}" class="cmd-item" role="option" aria-selected="false">
    ...
    async function showInitialQuickActions() {
      const data = await preloadSearchData();
      if (input.value.trim() === '') {
        const topActions = (data || []).slice(0, 5);
        renderList(topActions, 'Quick Actions & Ecosystem');
      }
    }
    ...
    dialog._showInitial = showInitialQuickActions;
    return dialog;
  }
